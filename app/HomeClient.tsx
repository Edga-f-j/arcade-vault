...
type ScoreRow = Pick<Tables<'scores'>, 'player_name' | 'score' | 'game_slug' | 'created_at'>
...
// ===== HomeClient =====
export default function HomeClient({ topScores }: { topScores: ScoreRow[] }) {
  const router = useRouter()
  useReveal()

  const displayScores = topScores.slice(0, 5)
  const maxScore = displayScores[0]?.score ?? 1
    ...
    <div className="home fade-in">
      ...
      <section className="home-section reveal">
        ...
        <div className="feature-grid">
          {[
            ...
            { i: 'ROCKET',  t: 'SIEMPRE CRECIENDO', d: 'Agregamos nuevos juegos constantemente. Vuelve seguido, siempre habrá algo nueva que jugar.',         c: 'green'   },
          ].map((f, i) => (
      ...
      <section className="home-section reveal">
        ...
        <div className="activity-grid">
          ...
          <div className="activity-card">
            <div className="ac-head">
              <div className="ac-title pixel neon-magenta">▸ TOP JUGADORES · GLOBAL</div>
              <button className="lb-link" onClick={() => router.push('/salon')}>VER SALÓN →</button>
            </div>
            {displayScores.length === 0 ? (
              <div style={{ padding: '24px 0', textAlign: 'center', color: 'var(--ink-faint)', fontSize: 12 }}>
                Aún no hay puntuaciones. ¡Sé el primero!
              </div>
            ) : (
              <div className="top-list">
                {displayScores.map((r, i) => (
                  <div key={r.player_name + i} className={`top-row${i === 0 ? ' top1' : i === 1 ? ' top2' : i === 2 ? ' top3' : ''}`}>
                    <span className="tp-rk">#{String(i + 1).padStart(2, '0')}</span>
                    <span className="tp-bar">
                      <span className="tp-fill" style={{ width: `${Math.round((r.score / maxScore) * 100)}%` }}></span>
                    </span>
                    <span className="tp-p">{r.player_name}</span>
